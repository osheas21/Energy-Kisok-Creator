...
struct PVData
{
    ...
    float moduleMaxPower;	// maximum power at STC for individual module
	float arrayMaxPower;	// maximum power at STC for entire array
    float moduleVoc;		// open-circuit voltage of individual module
	...
	float price;			// price of solar panel
};
...
struct BatteryData
{
	...
	float dischargeTime[5];				// hour-rate of the corresponding charge capacity [h]
    float moduleEffectiveCapacity[5];	// discharge rating corresponding to discharge rate of individual battery [Ah]
    float moduleDischargeCurrent[5];	// discharge rate corresponding to discharge time of indivudual battery [A]
	...
	float moduleVoltage;				// voltage of individual battery module
	float arrayVoltage;					// voltage of entire array
	float moduleNominalCapacity;		// capacity of invividual battery module
	...
	float price;						// price of individual battery
};
...
struct DataValues
{
	...
	int numChargeCycles;									// number of charge cycles the battery goes through each year
...
// initializes module/arrayEffectiveCapacity, dischargeTime, and module/arrayDischargeCurrent arrays to 0
void initializeBattery(struct BatteryData *battery);

// calculates the arrayEffectiveCapacity, moduleDischargeCurrent, arrayDischargeCurrent, arrayNominalCapacity,
// and arrayVoltage values using the parameters for the size of the array and parameters of indivudual battery modules
...
// array can be an array or a single value.
...
void importLoadProfile (FILE *load_profile, int *load);
...
// fills out the battery struct with the data entered in the csv library for battery modules
...
// stores the data downloaded from NASA POWER into various arrays
...
// calculates average extraterrestrial irradiance on a tilted surface over the course of an hour
...
// calculate the power produced by a PV module using the temperature of the module
// irradiance, and its characteristics
float calcPVPower (float stcPower, float irradiance, float alphaP, float cellTemp);

// calculates the output of the charge controller
float calcCControllerOutput (struct DataValues *data, struct ChargeControllerData *cController, float pvOutput);
...
// if the state of charge is greater than 97% and drops below 97%, one charge cycle is added to data.numChargeCycles
void calcChargeCycles (struct posdata *pdat, struct DataValues *data, float startSoC, float endSoC);

// calculates and stores the maximum state of charge for each month, each year, and all months of each year into
// the corresponding DataValues
void calcMaxSoC (struct posdata *pdat, struct DataValues *data);

// calculates and stores the minimum state of charge for each month, each year, and all months of each year into
// the corresponding DataValues
void calcMinSoC (struct posdata *pdat, struct DataValues *data);

// calculates the lower end of 6 evenly-sized state of charge ranges
...
// calculates the year that had the lowest pv output for each month
void lowestPVOutput (struct posdata *pdat, struct DataValues *data, float ***pvPower);

// reads the names of the models stored in the .csv files in the "Library" folder
void readModelNames (FILE *current_file, char *modelNames, int string_size, int num_strings);

// calls functions that store the results of the calculations into the *data struct
void storeData (struct posdata *pdat, struct DataValues *data, struct BatteryData *battery, float ***pvPower);

// calculates the average daily insolation on a horizontal surface in the specified location
