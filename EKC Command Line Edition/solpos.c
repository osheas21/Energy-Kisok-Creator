...
void initializeBattery(struct BatteryData *battery)
{
	...
	for (int i = 0; i < ARRAY_SIZE; i++) {
		battery->moduleEffectiveCapacity[i] = battery->dischargeTime[i] = battery->moduleDischargeCurrent[i] =
...
{
    const int ARRAY_SIZE = 5;
	battery->arrayNominalCapacity = battery->moduleNominalCapacity * battery->numStrings;
	battery->arrayVoltage = battery->moduleVoltage * battery->stringSize;
	for (int i = 0; i < ARRAY_SIZE; i++) {
		battery->moduleDischargeCurrent[i] = battery->moduleEffectiveCapacity[i] / battery->dischargeTime[i];	// module current = capacity[Ah] / time[h]
		battery->arrayDischargeCurrent[i] = battery->moduleDischargeCurrent[i] * battery->numStrings;			// array current = module current * number of strings
		battery->arrayEffectiveCapacity[i] = battery->moduleEffectiveCapacity[i] * battery->numStrings * battery->stringSize;
	}
}
...
void importLoadProfile (FILE *load_file, int *load)
{
    char line[100];
    char delim[2] = ",";
    char str[200];                              // allocate memory
	...
	while (fgets(line, sizeof(line), battery_file)) {
        ...
        if (currentModel == selection) {
            if (strcmp(curLabel, model) == 0)
                ...
                loadValues(line, &battery->moduleVoltage);

            else if (strcmp(curLabel, capacity) == 0)
                loadValues(line, &battery->moduleNominalCapacity);

            else if (strcmp(curLabel, dischargeTime) == 0)
                loadValues(line, battery->dischargeTime);

            else if (strcmp(curLabel, effectiveCapacity) == 0) {
                loadValues(line, battery->moduleEffectiveCapacity);

                if (battery->moduleEffectiveCapacity[0] == 1) {
                    for (int i = 0; i < 5; i++)
                        battery->moduleEffectiveCapacity[i] *= battery->moduleNominalCapacity;
                }
            }
	...
	while (fgets(line, sizeof(line), pv_file)) {
        ...
        if (currentModel == selection) {
            ...
            if (strcmp(curLabel, model) == 0)
                ...
                loadValues(line, &panel->moduleMaxPower);

            else if (strcmp(curLabel, Voc) == 0)
                loadValues(line, &panel->moduleVoc);
...
{
    char data[5];
    for (int i = 0; i < 5 && strcmp(&line[i], ",") != 0; i++) {		// for at least 5 digits, then until a comma is reached
        data[i] = line[i+18];										// data = the the data in the line being read
    }
...
{
    char year[5];
    char endOfSection[8] = "     },";	// what the last line in every section except the last section looks like
    char endOfData[7] = "     }";		// what the last line in the last section looks like
...
{
	float avgI = 0;
	float numTimeSteps = 4;		// number of data points to be averaged per hour. 4 => average of minutes 0, 15, 30, 45
    float increment = 60 / numTimeSteps;

	for (pdat->minute = 0; pdat->minute < 60; pdat->minute += increment) {
		S_solpos(pdat);
		avgI += pdat->etrtilt;
	}
    pdat->minute = 0;	// reset to 0 so that other functions that use pdat can assume minute == 0
...
float calcCControllerOutput (struct DataValues *data, struct ChargeControllerData *cController, float pvPower)
{
	float power = pvPower;
	if (power > cController->capacity)
		power = cController->capacity;

	power *= cController->efficiency;
...
{
    // offset "b" and .00346 derived from data in "inverter math.xlsx".
...
float calcBatteryCharge (struct BatteryData *battery, struct InverterData *inverter, float startSoC, float currentOut)
{
    ...
    if (currentOut > 0) {      // if discharging
		...
		currentOut += inverter->zeroLoadPower / battery->arrayVoltage;	// accounting for the power consumed by the inverter
	...
	// if battery is more than 100% charged, set to 100%. If less than min state of charge, set to min state of charge.
	if (endSoC > 1)
		endSoC = 1;
...
void calcChargeCycles (struct posdata *pdat, struct DataValues *data, float startSoC, float endSoC)
{
	if (startSoC > .97 && endSoC <= .97)
		data->numChargeCycles++;

	if (pdat->year == 2005 && pdat->daynum == 365 && pdat->hour == 23)
		data->numChargeCycles /= NUM_YEARS;
}
...
void calcSoCIntervals (struct posdata *pdat, struct DataValues *data, float minSoC)
{
	...
	for (int i = 0; i < numIntervals; i++) {
		...
		data->SoCIntervalProportion[i] = (float)data->SoCIntervalTime[i] / 8760.0;		// (number of hours at state of charge) / (num hours per year)
	}
}
...
void saveData (struct posdata *pdat, struct DataValues *data, float ***pvPower, float ***hourlyTemp, FILE *savefile)
{
	fprintf (savefile, "Latitude, %f, Longitude, %f\n", LATITUDE, LONGITUDE);

	fprintf (savefile, "Year, Month, Day, Daynum, Hour, PV Power, SoC, Ambient Temperature\n");
	for (pdat->year = START_YEAR; pdat->year < START_YEAR + NUM_YEARS; pdat->year++) {
		for (pdat->daynum = 1; pdat->daynum < 366 || (pdat->daynum < 367 && pdat->year % 4 == 0); pdat->daynum++) {   // while the day is less than 12/31 on a leap-year, and it is not the 366th day of the year on a non-leap-year
			doy2dom(pdat);
			for (pdat->hour = 0; pdat->hour < NUM_HOURS; pdat->hour++) {
				fprintf(savefile, "%i,%i,%i,%i,%i,%f,%f,%f\n", pdat->year, pdat->month, pdat->day, pdat->daynum, pdat->hour,
							pvPower[pdat->year-START_YEAR][pdat->daynum][pdat->hour], data->hourlySoC[pdat->year-START_YEAR][pdat->daynum][pdat->hour],
							hourlyTemp[pdat->year-START_YEAR][pdat->daynum][pdat->hour]);
			}
		}
	}
}